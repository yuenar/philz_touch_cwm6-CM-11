...
int nandroid_backup(const char* backup_path) {
    ...
    if (ensure_path_mounted(backup_path) != 0) {
        return print_and_error("不能挂载备份路径.\n", NANDROID_ERROR_GENERAL);
    }
    ...
    if (check_backup_size(backup_path) < 0)
        return print_and_error("没有足够的可用备份空间: 取消备份.\n", NANDROID_ERROR_GENERAL);
    ...
    if (vol != NULL) {
        if (is_custom_backup && backup_preload) {
            if (0 != (ret = nandroid_backup_partition(backup_path, "/preload"))) {
                return print_and_error("备份 /preload 失败!\n", ret);
            }
    ...
    if (backup_efs && vol != NULL) {
        ...
        if (0 != dd_raw_backup_handler(tmp, "/efs"))
            ui_print("EFS raw image备份出错! 尝试传统备份...\n");
...
int nandroid_restore_partition_extended(const char* backup_path, const char* mount_point, int umount_when_finished) {
    ...
    if (strcmp(backup_path, "-") == 0) {
        ...
        if (twrp_backup_mode.value) {
            ...
            ui_print("未发现 %s 备份 (img, tar, dup). 还原已略过 %s.\n", name, mount_point);
            return 0;
        } else {
            printf("发现新的备份镜像: %s\n", tmp);
        }
...
int nandroid_restore(const char* backup_path, int restore_boot, int restore_system, int restore_data, int restore_cache, int restore_cust, int restore_wimax) {
    ...
    if (enable_md5sum.value && verify_nandroid_md5sum(backup_path) != 0) {
        return print_and_error("MD5核对出错!\n", NANDROID_ERROR_GENERAL);
    }
    ...
    if (restore_wimax && vol != NULL && 0 == statfs(vol->blk_device, &s)) {
        ...
        if (0 != stat(tmp, &st)) {
            ...
            ui_print("WiMAX还原前需擦除...\n");
            if (0 != (ret = format_volume("/wimax")))
                return print_and_error("格式化wimax分区出错!\n", ret);
            ui_print("还原WiMAX镜像中...\n");
            if (0 != (ret = restore_raw_partition(vol->fs_type, vol->blk_device, tmp)))
                return print_and_error(NULL, ret);
        }
