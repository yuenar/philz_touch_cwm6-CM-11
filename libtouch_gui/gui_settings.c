...
static void browse_background_image() {
    ...
    const char* headers[] = {"选择一个图片或颜色", "", NULL};
...
static void change_menu_color() {
    const char* headers[] = { "改变菜单栏颜色", NULL };
...
static void screen_shot() {
    ...
    if (ret == -2) {
        ...
        ui_print("自定义图像资源已检测到:退出fb2png模式:\n");
    }
    if (ret == 0) {
...
static void gestures_action_setup() {
    const char* headers[] = { "手势行为设置", NULL };
    ...
    for (;;) {
        ...
        while (gesture_action[i] != NULL) {
            ui_format_gui_menu(item_slide_left, "左右滑动", gesture_action[slide_left_action.value]);
            ui_format_gui_menu(item_slide_right, "正确的滑动", gesture_action[slide_right_action.value]);
...
static void change_date_time_menu() {
    ...
    const char* headers[] = { "改变日期与时间:", chosen_date, "", NULL };
...
void show_touch_gui_menu() {
    const char* headers[] = { "GUI滑动触摸设置", NULL };
    ...
    for (;;) {
        if (touch_to_validate.value == NO_TOUCH_SUPPORT)
            ui_format_gui_menu(item_touch, "GUI滑动触摸", "禁用");
        else if (touch_to_validate.value == TOUCH_HIGHLIGHT_ONLY)
            ui_format_gui_menu(item_touch, "GUI滑动触摸", "仅选择");
        else if (touch_to_validate.value == DOUBLE_TAP_VALIDATION)
            ui_format_gui_menu(item_touch, "GUI滑动触摸", "双击");
        else
            ui_format_gui_menu(item_touch, "GUI滑动触摸", "全部");
        ...
        ui_format_gui_menu(item_height, "菜单的高度", tmp);
        ...
        ui_format_gui_menu(item_scroll, "滚动的敏感性", tmp);
        ...
        if (enable_vibrator.value)
            ui_format_gui_menu(item_vibra, "震动器", "启用");
        else ui_format_gui_menu(item_vibra, "震动器", "禁用");

        if (boardEnableKeyRepeat.value)
            ui_format_gui_menu(item_keyrep, "按键重复", "启用");
        else ui_format_gui_menu(item_keyrep, "按键重复", "禁用");

        if (show_menu_separation.value)
            ui_format_gui_menu(item_separator, "菜单分隔", "启用");
        else ui_format_gui_menu(item_separator, "菜单分隔", "禁用");

        if (wait_after_install.value)
            ui_format_gui_menu(item_pauseLog, "暂停日记", "启用");
        else ui_format_gui_menu(item_pauseLog, "暂停日记", "禁用");
        ...
        ui_format_gui_menu(item_logrows, "设置日记行", tmp);
        ...
        ui_format_gui_menu(item_dim_time, "昏暗的屏幕延迟", tmp);
        ...
        if (show_clock.value && show_battery.value)
            ui_format_gui_menu(item_batt_clock, "电池/时钟", "两者");
        else if (show_clock.value)
            ui_format_gui_menu(item_batt_clock, "电池/时钟", "时钟");
        else if (show_battery.value)
            ui_format_gui_menu(item_batt_clock, "电池/时钟", "电池");
        else ui_format_gui_menu(item_batt_clock, "电池/时钟", "禁用");
...
static int make_update_zip(const char* source_path, const char* target_volume) {
    ...
    if (source_path == NULL) {
        ...
        backup_recovery = 0, backup_wimax = 0, backup_data = 0, backup_cache = 0, backup_cust = 0;
        nandroid_force_backup_format("tar");
        ret = nandroid_backup(tmp_path);
        nandroid_force_backup_format("");
        backup_recovery = 1, backup_wimax = 1, backup_data = 1, backup_cache = 1, backup_cust = 1;
        if (ret != 0) {
            LOGE("当创建nandroid镜像时失败!\n");
            return ret;
        }
    ...
    if (ret != 0) {
        return print_and_error("当生成zip镜像时失败!\n", ret);
    }

    finish_nandroid_job();
    ui_print("自定义ROM保存在%s/%s\n", target_volume, CUSTOM_ROM_PATH);
    return 0;
}
...
static void custom_rom_target_volume(const char* source_path) {
    ...
    const char* headers[] = {"选择自定义ROM目标", "", NULL};
    char* list[MAX_NUM_MANAGED_VOLUMES + 1];
    char list_prefix[] = "创建ROM到 ";
    char buf[80];
    memset(list, 0, sizeof(list));
    sprintf(buf, "%s%s", list_prefix, primary_path);
    list[0] = strdup(buf);
...
static void choose_nandroid_menu() {
    ...
    if (!file_found(tmp)) {
        ...
        sprintf(tmp, "是的 - 创建时使用 %s", BaseName(file));
        if (confirm_selection("是否创建ROM?", tmp))
...
void show_recovery_lock_menu() {
    ...
    for (;;) {
        ...
        switch (chosen_item) {
            ...
            case 1: {
                if (confirm_selection("禁用Recovery锁 ?", "是 - 禁用")) {
...
void print_libtouch_version(int onscreen) {
    if (onscreen)
        ui_print("GUI触摸版本: " EXPAND(LIBTOUCH_GUI_VERSION) "\n");
    else
        LOGI("GUI触摸版本: " EXPAND(LIBTOUCH_GUI_VERSION) "\n");
}
