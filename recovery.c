...
main(int argc, char **argv) {
    ...
    ui_print("编译作者 " EXPAND(宇达yuenar@HRT.team) "\n");
    ui_print("编译机型 " EXPAND(荣耀畅玩4X海思620版本) "\n");
    ui_print("论坛支持 " EXPAND(花粉俱乐部huafans.cn) "\n");
    LOGI("目标设备: " EXPAND(TARGET_COMMON_NAME) "\n");
#ifdef PHILZ_TOUCH_RECOVERY
    ...
    if (update_package != NULL) {
        ...
        if (status != INSTALL_SUCCESS) ui_print("用户数据清除失败.\n");
    } else if (wipe_cache) {
        if (wipe_cache && erase_volume("/cache")) status = INSTALL_ERROR;
        if (status != INSTALL_SUCCESS) ui_print("Cache缓存清除失败.\n");
    } else if (wipe_media) {
        if (is_data_media() && erase_volume("/data/media")) status = INSTALL_ERROR;
        if (status != INSTALL_SUCCESS) ui_print("内部存储清除失败.\n");
    } else if (sideload) {
