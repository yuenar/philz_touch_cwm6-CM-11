...
int ensure_path_mounted_at_mount_point(const char* path, const char* mount_point) {
    ...
    Volume* v = volume_for_path(path);
    if (v == NULL) {
        // silent failure for cust
        if (strncmp(path, "/cust", 7) != 0)
...
int format_volume(const char* volume) {
    ...
    // silent failure for non existing cust (when we factory reset)
    if (v == NULL) {
        if (strcmp(volume, "/cust") != 0)
            LOGE("unknown volume \"%s\"\n", volume);
        return -1;
    }

    // silent failure to format non existing cust when defined in recovery.fstab
    if (strcmp(volume, "/cust") == 0) {
        struct stat s;
        if (0 != stat(v->blk_device, &s)) {
            LOGI("Skipping format of cust\n");
            return -1;
        }
    }
...
int format_unknown_device(const char *device, const char* path, const char *fs_type) {
    ...
    // if this is cust:, don't worry about it if it does not exist.
    if (strcmp(path, "/cust") == 0) {
        struct stat st;
        Volume *vol = volume_for_path("/cust");
        if (vol == NULL || 0 != stat(vol->blk_device, &st)) {
            LOGI("No app2sd partition found. Skipping format of /cust.\n");
            return 0;
        }
    }
