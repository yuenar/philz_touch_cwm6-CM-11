...
void wipe_data_menu() {
    ...
    // check if we have a volume that can be partitionned (cust support)
    char* primary_path = get_primary_storage_path();
    char** extra_paths = get_extra_storage_paths();
    int num_extra_volumes = get_num_extra_volumes();
        ...
        switch (chosen_item) {
            ...
            case 2: {
                if (0 != ensure_path_mounted("/data"))
                    break;
                if (volume_for_path("/data/cust") != NULL)
                    ensure_path_mounted("/data/cust");
...
int ors_backup_command(const char* backup_path, const char* options) {
    ...
    backup_preload = 0, backup_data = 0, backup_cache = 0, backup_cust = 0;
    ...
    for (i=0; i<line_len; i++) {
        if (value1[i] == 'S' || value1[i] == 's') {
            ...
            if (nandroid_add_preload.value) {
                backup_preload = 1;
                ui_print("预设已开启nandroid设置.\n");
                ui_print("它将随系统分区/system\n");
            }
    ...
    if (fp != NULL) {
        while (fgets(script_line, SCRIPT_COMMAND_SIZE, fp) != NULL && ret_val == 0) {
            ...
            if (cindex != 0) {
                ...
                ui_print("命令行是: '%s' and there is no value\n", command);
            }
            if (strcmp(command, "install") == 0) {
                ...
                if (tok != NULL) {
                    ...
                    for (i=0; i<line_len; i++) {
                        if (value2[i] == 'S' || value2[i] == 's') {
                            ...
                            if (nandroid_add_preload.value) {
                                backup_preload = 1;
                                ui_print("预设已启用nandroid设置.\n");
                                ui_print("它将伴随恢复system分区的进程\n");
                            }
                    ...
                    backup_data = 1, backup_cache = 1, backup_cust = 1;
                    ignore_android_secure = 0;
                ...
                if (twrp_backup_mode.value)
                    ...
                    ret_val = nandroid_restore(value1, backup_boot, backup_system, backup_data, backup_cache, backup_cust, 0);
                ...
                tok = strtok(NULL, " ");
                strcpy(value2, tok);
                ui_print("设置功能在CWM中禁用 '%s' to '%s'\n", value1, value2);
            } else if (strcmp(command, "mkdir") == 0) {
                // Make directory (recursive)
                ui_print("创建文件夹功能在CWM中禁用: '%s'\n", value);
...
static void choose_default_ors_menu(const char* volume_path) {
    ...
    char confirm[PATH_MAX];
    sprintf(confirm, "是的 - 运行 %s", BaseName(ors_file));
    if (confirm_selection("确认执行?", confirm)) {
...
static void choose_custom_ors_menu(const char* volume_path) {
    ...
    const char* headers[] = {"选择.ors 脚本来执行", NULL};
...
static void show_custom_ors_menu() {
    ...
    const char* headers[] = { "寻找.ors脚本来执行", "", NULL };
...
void get_rom_name(char *rom_name) {
    ...
    strcpy(rom_name, "未命名的");
    while ((key = rom_id_key[i]) != NULL && strcmp(rom_name, "未命名的") == 0) {
        if (read_config_file("/system/build.prop", key, rom_name, "未命名的") < 0) {
...
static void regenerate_md5_sum_menu() {
    if (!confirm_selection("这个操作不被推荐!!", "是的 - 重新生成md5值"))
        return;
    ...
    char list_prefix[] = "选择自 ";
    char buf[80];
    const char* headers[] = {"重新生成md5值", "选择一个备份来重新生成", NULL};
    char* list[MAX_NUM_MANAGED_VOLUMES + 1];
    memset(list, 0, sizeof(list));
    sprintf(buf, "%s%s", list_prefix, primary_path);
    list[0] = strdup(buf);
    ...
    sprintf(tmp, "处理中 %s", BaseName(backup_source));
    if (confirm_selection("是否重新生成md5值？", tmp)) {
...
void misc_nandroid_menu() {
    ...
    for (;;) {
        ...
        if (nand_prompt_on_low_space.value)
            ui_format_gui_menu(item_prompt_low_space, "内存空间不足时以提示", "(x)");
        else ui_format_gui_menu(item_prompt_low_space, "内存空间不足时以提示", "( )");
...
void get_cwm_backup_path(const char* backup_volume, char *backup_path) {
    char rom_name[PROPERTY_VALUE_MAX] = "未命名的";
...
static void custom_restore_handler(const char* backup_volume, const char* backup_folder) {
    ...
    if (backup_efs == RESTORE_EFS_IMG) {
        ...
        if (file_found(tmp)) {
            ui_print("efs.img 文件已检测到 %s!\n", file);
            ui_print("选择另外的efs.img以还原,\n");
        ...
        if (confirm_selection(confirm_install, tmp)) {
            nandroid_restore(backup_source, backup_boot, backup_system, backup_data, backup_cache, backup_cust, backup_wimax);
        }
    }
...
static void validate_backup_job(const char* backup_volume, int is_backup) {
    int sum = backup_boot + backup_recovery + backup_system + backup_preload + backup_data +
                backup_cache + backup_cust + backup_wimax + backup_misc + backup_data_media;
...
void custom_restore_menu(const char* backup_volume) {
    ...
    for (;;)
    {
        ...
        list[LIST_ITEM_VALIDATE] = strdup(">> 开始自定义还原工作");

        if (reboot_after_nandroid) ui_format_gui_menu(menu_item_tmp, ">> 是否完成后重启", "(x)");
        else ui_format_gui_menu(menu_item_tmp, ">> 是否完成后重启", "( )");
        ...
        if (volume_for_path(BOOT_PARTITION_MOUNT_POINT) != NULL) {
            ...
            else ui_format_gui_menu(menu_item_tmp, "还原 boot", "( )");
        ...
        if (volume_for_path("/recovery") != NULL) {
            if (backup_recovery) ui_format_gui_menu(menu_item_tmp, "还原 recovery", "(x)");
            else ui_format_gui_menu(menu_item_tmp, "还原 recovery", "( )");
            list[LIST_ITEM_RECOVERY] = strdup(menu_item_tmp);
        } else {
            list[LIST_ITEM_RECOVERY] = NULL;
        }

        if (backup_system) ui_format_gui_menu(menu_item_tmp, "还原 system", "(x)");
        else ui_format_gui_menu(menu_item_tmp, "还原 system", "( )");
        list[LIST_ITEM_SYSTEM] = strdup(menu_item_tmp);

        if (volume_for_path("/preload") != NULL) {
            if (backup_preload) ui_format_gui_menu(menu_item_tmp, "还原 preload", "(x)");
            else ui_format_gui_menu(menu_item_tmp, "还原 preload", "( )");
            list[LIST_ITEM_PRELOAD] = strdup(menu_item_tmp);
        } else {
            list[LIST_ITEM_PRELOAD] = NULL;
        }

        if (backup_data) ui_format_gui_menu(menu_item_tmp, "还原 data", "(x)");
        else ui_format_gui_menu(menu_item_tmp, "还原 data", "( )");
        ...
        if (backup_cache) ui_format_gui_menu(menu_item_tmp, "还原 cache", "(x)");
        else ui_format_gui_menu(menu_item_tmp, "还原 cache", "( )");
        list[LIST_ITEM_CACHE] = strdup(menu_item_tmp);

        if (backup_cust) ui_format_gui_menu(menu_item_tmp, "还原 cust", "(x)");
        else ui_format_gui_menu(menu_item_tmp, "还原 cust", "( )");
        list[LIST_ITEM_CUST] = strdup(menu_item_tmp);

        if (volume_for_path("/modem") != NULL) {
            if (backup_modem == RAW_IMG_FILE)
                ui_format_gui_menu(menu_item_tmp, "还原 modem [.img]", "(x)");
            else if (backup_modem == RAW_BIN_FILE)
                ui_format_gui_menu(menu_item_tmp, "还原 modem [.bin]", "(x)");
        ...
        if (volume_for_path("/radio") != NULL) {
            if (backup_radio == RAW_IMG_FILE)
                ui_format_gui_menu(menu_item_tmp, "还原 radio [.img]", "(x)");
            else if (backup_radio == RAW_BIN_FILE)
                ui_format_gui_menu(menu_item_tmp, "还原 radio [.bin]", "(x)");
            else ui_format_gui_menu(menu_item_tmp, "还原 radio", "( )");
        ...
        if (volume_for_path("/efs") != NULL) {
            if (backup_efs == RESTORE_EFS_IMG)
                ui_format_gui_menu(menu_item_tmp, "还原 efs [.img]", "(x)");
            else if (backup_efs == RESTORE_EFS_TAR)
                ui_format_gui_menu(menu_item_tmp, "还原 efs [.tar]", "(x)");
            else ui_format_gui_menu(menu_item_tmp, "还原 efs", "( )");
        ...
        if (volume_for_path("/misc") != NULL) {
            if (backup_misc) ui_format_gui_menu(menu_item_tmp, "还原 misc", "(x)");
            else ui_format_gui_menu(menu_item_tmp, "还原 misc", "( )");
        ...
        if (is_data_media() && !twrp_backup_mode.value) {
            if (backup_data_media)
                ui_format_gui_menu(menu_item_tmp, "还原 /data/media", "(x)");
            else ui_format_gui_menu(menu_item_tmp, "还原 /data/media", "( )");
        ...
        if (volume_for_path("/wimax") != NULL && !twrp_backup_mode.value) {
            if (backup_wimax)
                ui_format_gui_menu(menu_item_tmp, "还原 WiMax", "(x)");
            else ui_format_gui_menu(menu_item_tmp, "还原 WiMax", "( )");
        ...
        for (i = 0; i < extra_partitions_num; ++i)
        {
            if (volume_for_path(extra_partition[i].mount_point) != NULL) {
                sprintf(tmp, "还原 %s", extra_partition[i].mount_point);
                if (extra_partition[i].backup_state)
        ...
        switch (chosen_item)
        {
            ...
            case LIST_ITEM_ANDSEC:
                ...
                if (!ignore_android_secure && get_num_extra_volumes() != 0)
                    ui_print("强制还原到第二存储, 保持一致 .android_secure文件夹\n");
                break;
...
void custom_backup_menu(const char* backup_volume)
{
    ...
    for (;;)
    {
        ...
        if (volume_for_path(BOOT_PARTITION_MOUNT_POINT) != NULL) {
            if (backup_boot) ui_format_gui_menu(menu_item_tmp, "备份 boot", "(x)");
            else ui_format_gui_menu(menu_item_tmp, "备份 boot", "( )");
            list[LIST_ITEM_BOOT] = strdup(menu_item_tmp);
        } else {
            list[LIST_ITEM_BOOT] = NULL;
        }
        ...
        if (volume_for_path("/recovery") != NULL) {
            if (backup_recovery) ui_format_gui_menu(menu_item_tmp, "备份 recovery", "(x)");
            else ui_format_gui_menu(menu_item_tmp, "备份 recovery", "( )");
            list[LIST_ITEM_RECOVERY] = strdup(menu_item_tmp);
        } else {
            list[LIST_ITEM_RECOVERY] = NULL;
        }

        if (backup_system) ui_format_gui_menu(menu_item_tmp, "备份 system", "(x)");
        else ui_format_gui_menu(menu_item_tmp, "备份 system", "( )");
        list[LIST_ITEM_SYSTEM] = strdup(menu_item_tmp);

        if (volume_for_path("/preload") != NULL) {
            if (backup_preload) ui_format_gui_menu(menu_item_tmp, "备份 preload", "(x)");
            else ui_format_gui_menu(menu_item_tmp, "备份 preload", "( )");
            list[LIST_ITEM_PRELOAD] = strdup(menu_item_tmp);
        } else {
            list[LIST_ITEM_PRELOAD] = NULL;
        }

        if (backup_data) ui_format_gui_menu(menu_item_tmp, "备份 data", "(x)");
        else ui_format_gui_menu(menu_item_tmp, "备份 data", "( )");
        list[LIST_ITEM_DATA] = strdup(menu_item_tmp);
        ...
        if (backup_cache) ui_format_gui_menu(menu_item_tmp, "备份 cache", "(x)");
        else ui_format_gui_menu(menu_item_tmp, "备份 cache", "( )");
        list[LIST_ITEM_CACHE] = strdup(menu_item_tmp);

        if (backup_cust) ui_format_gui_menu(menu_item_tmp, "备份 cust", "(x)");
        else ui_format_gui_menu(menu_item_tmp, "备份 cust", "( )");
        list[LIST_ITEM_CUST] = strdup(menu_item_tmp);

        if (volume_for_path("/modem") != NULL) {
            if (backup_modem == RAW_IMG_FILE)
                ui_format_gui_menu(menu_item_tmp, "备份 modem [.img]", "(x)");
            else if (backup_modem == RAW_BIN_FILE)
                ui_format_gui_menu(menu_item_tmp, "备份 modem [.bin]", "(x)");
            else ui_format_gui_menu(menu_item_tmp, "备份 modem", "( )");
            list[LIST_ITEM_MODEM] = strdup(menu_item_tmp);
        } else {
            list[LIST_ITEM_MODEM] = NULL;
        }
        ...
        if (volume_for_path("/radio") != NULL) {
            if (backup_radio == RAW_IMG_FILE)
                ui_format_gui_menu(menu_item_tmp, "备份 radio [.img]", "(x)");
            else if (backup_radio == RAW_BIN_FILE)
                ui_format_gui_menu(menu_item_tmp, "备份 radio [.bin]", "(x)");
            else ui_format_gui_menu(menu_item_tmp, "备份 radio", "( )");
            list[LIST_ITEM_RADIO] = strdup(menu_item_tmp);
        } else {
            list[LIST_ITEM_RADIO] = NULL;
        }
        ...
        if (volume_for_path("/efs") != NULL) {
            if (backup_efs == RESTORE_EFS_IMG)
                ui_format_gui_menu(menu_item_tmp, "备份 efs [.img]", "(x)");
            else if (backup_efs == RESTORE_EFS_TAR)
                ui_format_gui_menu(menu_item_tmp, "备份 efs [.tar]", "(x)");
            else ui_format_gui_menu(menu_item_tmp, "备份 efs", "( )");
            list[LIST_ITEM_EFS] = strdup(menu_item_tmp);
        } else {
            list[LIST_ITEM_EFS] = NULL;
        }
        ...
        if (volume_for_path("/misc") != NULL) {
            if (backup_misc) ui_format_gui_menu(menu_item_tmp, "备份 misc", "(x)");
            else ui_format_gui_menu(menu_item_tmp, "备份 misc", "( )");
            list[LIST_ITEM_MISC] = strdup(menu_item_tmp);
        } else {
            list[LIST_ITEM_MISC] = NULL;
        }
        ...
        if (volume_for_path("/wimax") != NULL && !twrp_backup_mode.value) {
            if (backup_wimax)
                ui_format_gui_menu(menu_item_tmp, "备份 WiMax", "(x)");
            else ui_format_gui_menu(menu_item_tmp, "备份 WiMax", "( )");
            list[LIST_ITEM_WIMAX] = strdup(menu_item_tmp);
        } else {
            list[LIST_ITEM_WIMAX] = NULL;
        }
        ...
        switch (chosen_item)
        {
            ...
            case LIST_ITEM_ANDSEC:
                ...
                ignore_android_secure ^= 1;
                if (!ignore_android_secure && get_num_extra_volumes() != 0)
                    ui_print("强制备份到第二存储, 保持一致 .android_secure文件夹\n");
                break;
            case LIST_ITEM_CACHE:
                backup_cache ^= 1;
                break;
...
static void load_theme_settings() {
...
#else
    const char* headers[] = { "选择一主题以载入", "", NULL };
    ...
    if (confirm_selection("是否重写默认设置 ?", "是的 - 应用新的主题") && copy_a_file(theme_file, PHILZ_SETTINGS_FILE) == 0) {
...
static void import_export_settings() {
    const char* headers[] = { "保存或还原设置", "", NULL };

    char* list[] = {
        "备份 Recovery设置到sdcard",
        "还原 Recovery设置从sdcard",
        "保存正确的主题到sdcard",
        "载入已保存在sdcard的主题",
        "删除已保存的主题",
        NULL
    ...
    for (;;) {
        ...
        switch (chosen_item) {
            ...
            case 4: {
                ...
                if (theme_file == NULL)
                    break;
                if (confirm_selection("删除所选主题?", "是的，删除"))
    ...
    for (;;) {
        ...
        switch (chosen_item) {
            ...
            case 6: {
                ...
                ui_print("基于CWM版本: " EXPAND(CWM_BASE_VERSION) "\n");
                ui_print("编译作者: HRT.team_宇达yuenar\n");
                ui_print("新浪微博: 最范儿的孬Now\n");
#ifdef PHILZ_TOUCH_RECOVERY
                print_libtouch_version(1);
#endif
                //ui_print(EXPAND(BUILD_DATE)"\n");
                ui_print("编译日期%s at %s\n", __DATE__, __TIME__);
                break;
            }
        }
    }
    ...
    for (;;) {
        if (is_data_media()) {
            if (use_migrated_storage())
                list[6] = "将内部存储挂载为: /data/media/0";
            else list[6] = "将内部存储挂载为: /data/media";
        }
        ...
        if (enabled < 0) {
            ...
            if (enabled)
                ui_format_gui_menu(item_loki_toggle_menu, "是否应用Loki补丁", "(x)");
            else
                ui_format_gui_menu(item_loki_toggle_menu, "是否应用Loki补丁", "( )");
        ...
        switch (chosen_item) {
            ...
            case 2: {
                if (confirm_selection("移除存在的su ?", "是的 - 应用SuperSU")) {
            ...
            case 6: {
                if (is_data_media() && ensure_path_mounted("/data") == 0) {
                    if (use_migrated_storage()) {
                        ...
                        ui_print("内部存储已设定为  /data/media/0\n");
                    }
...
int verify_root_and_recovery() {
    ...
    if (0 != lstat("/system/etc/.installed_su_daemon", &st)) {
        ...
        if (0 == lstat("/system/etc/install-recovery.sh", &st)) {
            if (st.st_mode & (S_IXUSR | S_IXGRP | S_IXOTH)) {
                ui_SetShowText(true);
                if (confirm_selection("该ROM可能恢复官方recovery. 修复?", "是的，禁止还原官方recovery")) {
    ...
    if (0 == lstat("/system/bin/su", &st)) {
        ...
        if (S_ISREG(st.st_mode)) {
            ...
            if (needs_suid && (st.st_mode & (S_ISUID | S_ISGID)) != (S_ISUID | S_ISGID)) {
                ui_SetShowText(true);
                if (confirm_selection("Root权限可能丢失，修复?", "是的，修复ROOT (/system/bin/su)")) {
    ...
    if (0 == lstat("/system/xbin/su", &st)) {
        ...
        if (S_ISREG(st.st_mode)) {
            ...
            if (needs_suid && (st.st_mode & (S_ISUID | S_ISGID)) != (S_ISUID | S_ISGID)) {
                ui_SetShowText(true);
                if (confirm_selection("Root权限可能丢失，修复?", "是的，修复ROOT (/system/xbin/su)")) {
    ...
    if (!exists || su_nums != 1) {
        ui_SetShowText(true);
        if (confirm_selection("Root权限已丢失，再次获取?", "是的，重新ROOT (/system/xbin/su)")) {
